...
DOCUMENTATION = r"""
---
module: netbox_module_interface_template
short_description: Creates or removes interfaces on modules from NetBox
description:
...
options:
  data:
    ...
    suboptions:
      module_type:
        description:
          - Name of the module the interface template will be associated with (case-sensitive)
        required: true
        type: raw
      name:
        description:
...
- name: "Test NetBox interface template module"
  ...
  tasks:
    - name: Create interface template within NetBox with only required information
      netbox.netbox.netbox_module_interface_template:
        netbox_url: http://netbox.local
        netbox_token: thisIsMyToken
        data:
          module_type: Arista Test
          name: 10GBASE-T (10GE)
          type: 10gbase-t
        state: present
...
def main():
    ...
    required_if = [
        ("state", "present", ["module_type", "name", "type"]),
        ("state", "absent", ["module_type", "name", "type"]),
    ]
    ...
    netbox_module_interface_template = NetboxDcimModule(module, NB_INTERFACE_TEMPLATES)
    netbox_module_interface_template.run()
